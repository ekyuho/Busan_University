...
#define BANNER "In-Door Ambient Environment Monitoring by  Kyuho_Kim"
...
#define DEFAULT1 "{\"user\":1200,\"topic\":\"s2m\",\"device\":\"ambient\",\"target\":\"busan university\",\"ssid\":\"\",\"password\":\"\",\"mqttserver\":\"damoa.io\"}"
...
#define DUSTRX 5
//#define DUSTRX 13 // only for 한자문버전
#include "screen.h"
Screen myscreen;
...
public:
	...
	void print() {
		//Serial.printf("\n > read co2: %dppm@%d",ppm,numreadings);
		Serial.printf(" C %d",ppm);
	}
private:
...
public:
	...
	int update(void) {
		...
		if (c == 32 && r[0]==header[0] && r[1]==header[1]) {
			...
			pm10=r[14]*256+r[15];
			//no stat
			//int k=append(pm25,pm10);
			//Serial.printf("\n > read dust: %d,%d@%d",pm25,pm10,k);
			#ifdef USE_BLUETOOTH
			SerialBT.printf("\nPM %d %d",pm25,pm10);
			#endif
			return 0;
	...
	void print() {
		//Serial.printf("\n > read dust: %d,%d@%d", pm25, pm10, numreadings-1);
		Serial.printf(" D0 %d D1 %d", pm25, pm10);
	}
private:
...
public:
	...
	int append(float reading[]) {
		...
		return numreadings; // NO BUFFERING ++;
	}
	...
	void update(void) {
		...
		//Serial.printf(" %.1f %.1f %.1f %.1f %.1f", reading[0], reading[1], reading[2],reading[3], reading[4]);


		//Serial.printf("\n > THVPA:");
		//Serial.printf("\n V0");
		//for (int i=0; i<5;i++) Serial.printf(" %.f", reading[i]);
		//Serial.printf(" %f", reading[2]);
...
public:
    ...
    void update(void) {
        ...
        if (error) {
            ...
            //Serial.printf(" > VN: %d %d @%d", srawVoc, srawNox, numreadings);
			//Serial.printf(" V1 %f N %f", srawVoc/1000., srawNox/1000.);

			readings[0][numreadings]=srawVoc/1000.;
			readings[1][numreadings]=srawNox/1000.;
...
public:
	...
	void update(void) {
		...
		int k= append(temp,humid);
		//Serial.printf("\n > read temp,humid: %.1f,%.1f@%d",temp,humid,k);
		#ifdef USE_BLUETOOTH
		SerialBT.printf("\nTH %.1f,%.1f",temp,humid);
		#endif
	}
	void print() {
		if (!ready) Serial.printf("\n > read temp,humid: %.1f,%.1f@%d",temp,humid,numreadings-1);
		Serial.printf("\n > read temp,humid: %.1f,%.1f@%d",temp,humid,numreadings-1);
	}
} mytemphumid;
#endif
...
public:
	...
	void begin() {
		...
		mytopic1s = String(myconf.topic)+"1s/"+myconf.user+"/";
	}
	...
	void publish1s(String specific, const char* message) { //ZZ
		if (connect()) {
			Serial.printf("\n >> %s %s", (mytopic1s+specific).c_str(), message);
			mqttClient.publish((mytopic1s+specific).c_str(), message);
		} else
			Serial.printf("\n failed to mqtt.connect()");
	}
...
void do_tick_1sec() {
	...
	//mytemphumid.print();
	//mydust.print();
	//sprintf(tbuf, "CTHDD %d,%.1f,%.1f,%d,%d", myco2.ppm, mytemphumid.temp, mytemphumid.humid, mydust.pm25,mydust.pm10);
	// BME680
	// temp, humid, gas, pressure, altitude
	// mybme.reading[0], mybme.reading[1],mybme.reading[2],mybme.reading[3],mybme.reading[4]
	// SGP41
	// Voc, Nox
	//mysgp.readings[0][0], mysgp.readings[1][0],
	
	sprintf(tbuf, "{\"T\":%.1f,\"H\":%.0f,\"Gas\":%f,\"Hpa\":%.0f,\"Alt\":%.1f,\"VOC\":%f,\"Nox\":%f,\"CO2\":%d,\"PM25\":%d,\"PM10\":%d,\"serial\":%d}", 
		mybme.reading[0], mybme.reading[1],mybme.reading[2],mybme.reading[3],mybme.reading[4], 
		mysgp.readings[0][0], mysgp.readings[1][0],
		myco2.ppm, mydust.pm25, mydust.pm10,serial++);
	if (strlen(tbuf)>MQTT_MAX_PACKET_SIZE) {
		Serial.printf("\n pubsub buffer overflow");
	}
	myled.on();
	mymqtt.publish1s("data", tbuf);
	myled.off();
	//Serial.printf("\n published %s", tbuf);
}
...
void setup() {
	Serial.begin(115200);
	delay(250);
	Serial.printf("\n\n\n\nKyuho's In-Door Ambient Environment Monitoring Platform is beginning... Yeah!");
	Serial.printf("\n%s %s", BANNER, VERSION);
...
void main_loop(void *param) { Serial.printf("\n# main_loop task running on core %d", xPortGetCoreID()); delay(1000); while (1) {
	...
	if (false) if (mark<millis()) {
		mark = millis()+2000;
		do_tick_2sec();
	}

	if (false) if (minute< millis()) {
		minute=millis()+60000;
		mark4=minute - 3000; //3초전
		do_tick_60sec();
	}
	if (false) if (mark4 && mark4<millis()) {
		mark4=0;
		do_tick_60minus3sec();
	}
	mqttClient.loop();
